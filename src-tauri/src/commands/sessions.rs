...
pub fn get_sessions(
    ...
    if let Some(ref search) = params.search {
        conditions.push(
            "(s.first_prompt LIKE ? OR s.custom_title LIKE ? OR s.ai_title LIKE ?)".to_string(),
        );
    ...
    let data_sql = format!(
        "SELECT s.id, s.project_id, s.file_path, s.file_mtime, s.file_size,
                s.first_prompt, s.custom_title, s.ai_title, s.message_count,
                s.user_message_count, s.assistant_message_count, s.tool_use_count,
                s.git_branch, s.cwd, s.model, s.version, s.permission_mode,
                s.is_sidechain, s.is_active, s.active_status, s.active_updated_at,
